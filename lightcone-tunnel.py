...
Production Release v1.2.0 (Full Path TCP/UDP Burst Mode & RS-FEC Anti-Packet-Loss)
"""
...
# Network & Timeout Safety Constraints
MAX_PAYLOAD_SIZE = 1200        # Prevents UDP MTU IP fragmentation
...
# Anti-DPI Burst Mode Jitter Parameters
...
def get_chunk_size() -> int:
    """Returns a random integer between 800 and MAX_PAYLOAD_SIZE to eliminate fixed payload size fingerprints."""
...
class TunnelSecurity:
    """AEAD ChaCha20-Poly1305 Encryption, Deterministic Nonce & Sliding Anti-Replay."""
    ...
    def pack_and_encrypt(self, payload: bytes) -> bytes:
        ...
        # Anti-DPI Dynamic Padding (0-15 random bytes appended)
        pad_len = os.urandom(1)[0] % 16
        padding = os.urandom(pad_len)
        
        # Meta Header: RandomPrefix(4B) + Seq(8B) + Timestamp(8B) + PadLen(1B) = 21 Bytes
        random_prefix = os.urandom(4)
        meta_header = random_prefix + struct.pack("!QQB", self.seq_counter, timestamp, pad_len)
        
        # Deterministic 12-Byte Nonce: Timestamp(4B) + SeqCounter(8B) -> 0% Collision
    ...
    def decrypt_and_unpack(self, datagram: bytes) -> Optional[bytes]:
        if len(datagram) < 21 + 12 + 16:  # Meta Header(21B) + Nonce(12B) + Tag(16B)
            return None
        ...
        # Time-based Sliding Window Anti-Replay Check
        if seq in self.seen_sequences:
            return None
        self.seen_sequences[seq] = now

        # Periodic purge of stale sequence records (>30s)
...
class FECGroupEncoder:
    """FEC Encoder: Groups N ciphertext payloads, pads dynamically, and emits N+M datagrams."""
    ...
    def input_packet(self, ciphertext: bytes) -> List[bytes]:
        ...
        # Auto-flush incomplete group if idle for > 20ms to keep latency minimal
        ...
        # FEC Header: GroupID(8B) + ShardIdx(1B) + N(1B) + M(1B) + RawLen/MaxLen(2B) = 13 Bytes
        header = struct.pack("!QBBH", self.group_id, idx, self.n, self.m, len(ciphertext))
    ...
    def _flush_parity(self) -> List[bytes]:
        ...
        if zfec:
            ...
            # Simple XOR fallback if zfec is absent (M=1 target)
        ...
        for p_idx, p_data in enumerate(parity_blocks, start=self.n):
            header = struct.pack("!QBBH", self.group_id, p_idx, self.n, self.m, max_len)
...
class FECGroupDecoder:
    """FEC Decoder: Reconstructs lost ciphertexts using RS parity shards with Scheme B Sweeper."""
    def __init__(self, timeout_sec: float = 3.0):
        # Key: (peer_addr, group_id) -> dict
    ...
    def process_datagram(self, peer_addr: object, datagram: bytes, fec_enabled: bool) -> List[bytes]:
        ...
        group_id, idx, n, m, raw_len = struct.unpack("!QBBH", datagram[:13])
        payload = datagram[13:]
        now = time.time()
        ...
        # Instant zero-latency delivery for uncorrupted data shards
        if idx < n:
        ...
        # Trigger RS reconstruction when data loss occurs and enough shards exist
    ...
    def sweep_stale_groups(self):
        """Scheme B: Low-overhead lazy cleanup executed by periodic background sweeper."""
        now = time.time()
...
# ============================================================================
# Binary Multiplexing Frame Protocol Encoder / Decoder
# ============================================================================
class MultiplexFrame:
    """
    Binary Multiplexing Header Format:
    [Stream ID: 4B] [Cmd: 1B] [ATYP: 1B] [Host/Port Info] [Payload]
    """
    @staticmethod
    def pack(stream_id: int, cmd: int, atyp: int, host: str, port: int, payload: bytes) -> bytes:
        header = struct.pack("!IBB", stream_id, cmd, atyp)
        if atyp == ATYP_IPV4:
    ...
    @staticmethod
    def unpack(data: bytes) -> Tuple[int, int, int, str, int, bytes]:
        stream_id, cmd, atyp = struct.unpack("!IBB", data[:6])
        idx = 6
        host, port = "", 0
        ...
        return stream_id, cmd, atyp, host, port, data[idx:]
...
class ClientEngine:
    def __init__(self, config: dict):
        ...
        # Active TCP streams: stream_id -> (writer, last_active_time)
        self.streams: Dict[int, Tuple[asyncio.StreamWriter, float]] = {}
        # SOCKS5 UDP sessions: stream_id -> (client_udp_addr, local_udp_transport, last_active_time)
    ...
    async def cleanup_idle_loop(self):
        """Sweeps idle TCP streams, idle UDP sessions, and stale FEC group decoders."""
        while True:
    ...
    async def handle_socks5(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        try:
            ver, nmethods = struct.unpack("!BB", await reader.readexact(2))
            await reader.readexact(nmethods)
            writer.write(b"\x05\x00")  # NO AUTHENTICATION REQUIRED
            await writer.drain()

            ver, cmd, _, atyp = struct.unpack("!BBBB", await reader.readexact(4))
            
            if atyp == ATYP_IPV4:
                host = socket.inet_ntoa(await reader.readexact(4))
            elif atyp == ATYP_DOMAIN:
                d_len = (await reader.readexact(1))[0]
                host = (await reader.readexact(d_len)).decode('utf-8')
            elif atyp == ATYP_IPV6:
                host = socket.inet_ntop(socket.AF_INET6, await reader.readexact(16))
            port = struct.unpack("!H", await reader.readexact(2))[0]
            ...
            if cmd == 0x01:  # TCP CONNECT
                ...
                self.next_stream_id += 1
                self.streams[stream_id] = (writer, time.time())
                ...
                while True:
                    ...
                    if not data:
                        break
                    self.streams[stream_id] = (writer, time.time())
                    frame = MultiplexFrame.pack(stream_id, CMD_TCP_DATA, atyp, host, port, data)
            ...
            elif cmd == 0x03:  # UDP ASSOCIATE (SOCKS5 UDP Forwarding)
                ...
                class Socks5UDPRelay(asyncio.DatagramProtocol):
                    ...
                    def datagram_received(self, data, addr):
                        ...
                        if u_atyp == ATYP_IPV4:
                            ...
                            d_port = struct.unpack("!H", data[idx+16:idx+18])[0]
                            idx += 18
                        else:
                            return
                        ...
                        async def send_burst_udp():
                            if current_count % BURST_PACKET_COUNT == 0:
                                await asyncio.sleep(random.uniform(BURST_SLEEP_MIN, BURST_SLEEP_MAX))
                            frame = MultiplexFrame.pack(self.sid, CMD_UDP_DATA, u_atyp, d_host, d_port, u_payload)
            ...
            logging.debug(f"[SOCKS5] Client stream closed: {e}")
        finally:
            writer.close()
            if udp_transport:
                udp_transport.close()
            if stream_id is not None:
                ...
                if stream_id in self.streams:
                    ...
                    try:
                        frame = MultiplexFrame.pack(stream_id, CMD_TCP_CLOSE, atyp, host, port, b"")
                        self.send_to_server(frame)
                    except Exception:
                        pass
    ...
    async def handle_http_proxy(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        try:
            ...
            if method == "CONNECT":
                ...
                frame = MultiplexFrame.pack(stream_id, CMD_TCP_DATA, atyp, host, port, header_line)
                self.send_to_server(frame)
            ...
            logging.debug(f"[HTTP Proxy] Client stream closed: {e}")
        finally:
            writer.close()
        ...
        class ClientUDPProtocol(asyncio.DatagramProtocol):
            ...
            def datagram_received(self, data, addr):
                ...
                for decrypted_pkt in payloads:
                    ...
                    try:
                        stream_id, cmd, atyp, r_host, r_port, payload = MultiplexFrame.unpack(decrypted)
                    except Exception as e:
                        logging.debug(f"[Client Multiplex] Dropped malformed frame: {e}")
                        continue
                    
                    if cmd == CMD_TCP_DATA and stream_id in self.engine.streams:
                        writer, _ = self.engine.streams[stream_id]
                        try:
                            self.engine.streams[stream_id] = (writer, time.time())
                            writer.write(payload)
                        except Exception as e:
                            logging.debug(f"[Client Outbound] Write exception on stream {stream_id}: {e}")
                            self.engine.streams.pop(stream_id, None)
                            writer.close()
                            frame = MultiplexFrame.pack(stream_id, CMD_TCP_CLOSE, atyp, r_host, r_port, b"")
                            self.engine.send_to_server(frame)
...
class ServerEngine:
    def __init__(self, config: dict):
        ...
        # Active TCP Connections: stream_id -> (reader, writer, last_active_time)
        self.tcp_connections: Dict[int, Tuple[asyncio.StreamReader, asyncio.StreamWriter, float]] = {}
        # In-Flight Connection Requests: set of stream_ids currently attempting connection
        self.connecting_streams: Set[int] = set()
        # Full-Cone NAT UDP Table: stream_id -> (transport, last_active_time)
    ...
    async def cleanup_idle_loop(self):
        """Periodically cleans up idle TCP sockets, stale UDP NAT mappings, and FEC decoders."""
        while True:
            await asyncio.sleep(30)
            now = time.time()
            
            stale_tcp = [sid for sid, (_, _, last_act) in list(self.tcp_connections.items()) if now - last_act > IDLE_TIMEOUT_SEC]
    ...
    async def handle_outbound_tcp(self, stream_id: int, atyp: int, host: str, port: int, data: bytes, transport, client_addr):
        now = time.time()
        
        # State Check & Race Condition Shield
        if stream_id not in self.tcp_connections and stream_id not in self.connecting_streams:
            if len(self.tcp_connections) + len(self.connecting_streams) >= self.max_concurrent_streams:
                logging.warning(f"[Server Outbound] Connection limit reached ({self.max_concurrent_streams}), dropping stream {stream_id}")
                frame = MultiplexFrame.pack(stream_id, CMD_TCP_CLOSE, atyp, host, port, b"")
                self.send_to_client(transport, client_addr, frame)
                return

            self.connecting_streams.add(stream_id)
            try:
                reader, writer = await asyncio.wait_for(
                    asyncio.open_connection(host, port),
                    timeout=CONNECT_TIMEOUT_SEC
                )
                self.tcp_connections[stream_id] = (reader, writer, time.time())
            except Exception as e:
                logging.error(f"[Server Outbound] Connection failed ({host}:{port}): {e}")
                frame = MultiplexFrame.pack(stream_id, CMD_TCP_CLOSE, atyp, host, port, b"")
                self.send_to_client(transport, client_addr, frame)
                return
            finally:
                self.connecting_streams.discard(stream_id)

            # Background Task: Forward target response back to Client using Burst Mode
            async def pipe_back():
                packet_counter = 0
                try:
                    while True:
                        ...
                        if stream_id in self.tcp_connections:
                            _, w, _ = self.tcp_connections[stream_id]
                            self.tcp_connections[stream_id] = (reader, w, time.time())
                            
                        frame = MultiplexFrame.pack(stream_id, CMD_TCP_DATA, atyp, host, port, resp)
                        self.send_to_client(transport, client_addr, frame)
            ...
            asyncio.create_task(pipe_back())

        elif stream_id in self.connecting_streams:
            return

        # Normal Data Transmission: Proactive Close Notification on Write Failure
        conn = self.tcp_connections.get(stream_id)
        if conn:
            reader, writer, _ = conn
            self.tcp_connections[stream_id] = (reader, writer, now)
            try:
                writer.write(data)
                await writer.drain()
            except Exception as e:
                logging.debug(f"[Server Outbound] Write error on stream {stream_id}: {e}")
                conn = self.tcp_connections.pop(stream_id, None)
                if conn:
                    conn[1].close()
                frame = MultiplexFrame.pack(stream_id, CMD_TCP_CLOSE, atyp, host, port, b"")
                self.send_to_client(transport, client_addr, frame)
    ...
    async def handle_outbound_udp_fullcone(self, stream_id: int, host: str, port: int, data: bytes, transport, client_addr):
        ...
        if stream_id not in self.udp_nat_table:
            ...
            class FullConeUDPProtocol(asyncio.DatagramProtocol):
                def __init__(self, server_engine, sid):
                ...
                def datagram_received(self, resp_data, remote_addr):
                    ...
                    async def send_burst_udp_back():
                        ...
                        frame = MultiplexFrame.pack(
                            self.sid, CMD_UDP_DATA, r_atyp, r_ip, r_port, resp_data
                        )
                        self.engine.send_to_client(transport, client_addr, frame)

                    asyncio.create_task(send_burst_udp_back())

            udp_transport, _ = await loop.create_datagram_endpoint(
                lambda: FullConeUDPProtocol(self, stream_id), local_addr=("0.0.0.0", 0)
            )
        ...
        class ServerUDPProtocol(asyncio.DatagramProtocol):
            ...
            def datagram_received(self, data, addr):
                ...
                for decrypted_pkt in payloads:
                    ...
                    if cmd == CMD_TCP_DATA:
                        asyncio.create_task(
                            self.engine.handle_outbound_tcp(stream_id, atyp, r_host, r_port, payload, self.transport, addr)
                        )
...
def apply_resource_limits(limit_mb: int):
    """Optimizes POSIX File Descriptors for high concurrency."""
